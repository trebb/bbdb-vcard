...
;;  (setq str (bbdb-vcard-translate str t)
...
;;	result)
;;    (dolist (item list)
;;      (puthash (bbdb-vcard-escape-strings item) t hash))
...
;;    (concat 
;;     "adr;type=" (bbdb-vcard-escape-strings (bbdb-address-location address)) ":"
;;     ";;" ;; no post office box, no extended address
;;     (bbdb-join (bbdb-vcard-escape-strings streets) ",") ";"
;;     (bbdb-vcard-escape-strings city) ";"
;;     (bbdb-vcard-escape-strings state) ";"
;;     (bbdb-vcard-escape-strings zip) ";"
;;     (bbdb-vcard-escape-strings country))))
...
;;    (when aka
;;      (insert "nickname:" (bbdb-join (bbdb-vcard-escape-strings aka) ",") "\n"))
...
;;    (when company
;;      (insert "org:" (bbdb-vcard-escape-strings company) "\n"))
;;    (when notes
;;      (insert "note:" (bbdb-vcard-escape-strings notes) "\n"))
;;    (dolist (phone phones)
;;      (insert "tel;type=" (bbdb-vcard-escape-strings (bbdb-phone-location phone)) ":"
;;	      (bbdb-vcard-escape-strings (bbdb-phone-string phone)) "\n"))
;;    (dolist (address addresses)
;;      (insert (bbdb-vcard-vcardize-address address) "\n"))
;;    (dolist (mail net)
;;      (insert "email;type=internet:" (bbdb-vcard-escape-strings mail) "\n"))
;;    ;; Use CATEGORIES based on mail-alias.  One or more text values
;;    ;; separated by a COMMA character (ASCII decimal 44).
;;    (when categories
;;      (insert "categories:" 
;;	      (bbdb-join (mapcar 'bbdb-vcard-escape-strings
